...
- (void)closeAllTabs {
	...
	for (int i = [self count] - 1.f; i >= 0; --i)
...
//	std::min([self count], std::max(index, [self IndexOfFirstNonMiniTab]));
    return miniTab ? MIN(MAX(0, index), (int)[self indexOfFirstNonMiniTab]) : MIN((int)self.count, MAX(index, [self indexOfFirstNonMiniTab]));
}
...
- (void)selectLastTab {
	[self selectTabContentsAtIndex:[self count]-1.f
					   userGesture:YES];
}

- (void)moveTabNext {
	int newIndex = MIN(activeIndex_ + 1, (int)[self count] - 1);
...
- (BOOL)isContextMenuCommandEnabled:(int)contextIndex
	...
	switch (commandID) {
		...
		case CommandCloseOtherTabs: {
			...
			int nonMiniTabCount = (int)[self count] - miniTabCount;
...
- (NSArray *)getIndicesClosedByCommand:(ContextMenuCommand)commandID
	...
	for (int i = [self count] - 1.f; i >= start; --i) {
...
- (BOOL)isNewTabAtEndOfTabStrip:(CTTabContents *)contents {
	return !contents || contents == [self tabContentsAtIndex:([self count] - 1.f)];
...
- (void)selectRelativeTab:(BOOL)forward {
	...
	index = (index + (int)[self count] + delta) % [self count];
